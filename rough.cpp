...
void HAL_TIM_PWM_PulseFinishedCallback(TIM_HandleTypeDef *htim)
{
  ...
  {
    counter++;           // her kesmede counteri arttir.
    if (counter == step) // eger counter istenilen adim sayisi kadar artmis ise
    {
      HAL_TIM_PWM_Stop_IT(&htim1, TIM_CHANNEL_1); // PWM'i durdur.
      counter = 0;                                // bir sonraki komutta counter sifirdan baslamali
...
void Step(int adim, int direction) // step motor fonksiyonu. iki adet parametre alicak. Pals ve y�n parametreleri
{
  step = adim;
  if (direction == 0)
